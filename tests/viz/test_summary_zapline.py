"""Tests for ZapLine-related summary helpers and plots."""
...
class TestGetNRemoved:
    ...
    def test_n_removed_attr(self):
        ...
        assert _get_zapline_n_removed(obj) == 2
    ...
    def test_n_removed_fallback(self):
        ...
        assert _get_zapline_n_removed(R()) == 5

    def test_dict_input(self):
        assert _get_zapline_n_removed({"n_removed": 3}) == 3

    def test_missing(self):
        class R:
            pass

        assert _get_zapline_n_removed(R()) == 0
...
class TestGetRemoved:
    ...
    def test_direct_attr(self):
        obj = MockStandardZapLine()
        result = _get_zapline_removed(obj)
    ...
    def test_dict_input(self):
        data = np.ones((3, 10))
        result = _get_zapline_removed({"removed": data})
    ...
    def test_missing(self):
        ...
        assert _get_zapline_removed(R()) is None
...
# =====================================================================
# plot_zapline_adaptive_summary tests
# =====================================================================
...
class TestPlotAdaptiveSummary:
    ...
    def test_basic(self):
        obj = MockAdaptiveZapLine()
        fig = plot_zapline_adaptive_summary(obj, show=False)
    ...
    def test_with_psd_data(self):
        ...
        fig = plot_zapline_adaptive_summary(
            obj, data_before=before, data_after=after, sfreq=500, show=False
        )
        assert isinstance(fig, plt.Figure)

    def test_no_chunk_info_placeholder_chunk(self):
        """When chunk_info is empty, the adaptive summary should still render."""
        obj = MockAdaptiveZapLine()
    ...
    def test_custom_title(self):
        obj = MockAdaptiveZapLine()
        fig = plot_zapline_adaptive_summary(obj, title="Custom Title", show=False)
        assert isinstance(fig, plt.Figure)

    def test_with_channel_names(self):
        obj = MockAdaptiveZapLine()
        names = [f"CH{i}" for i in range(8)]
        fig = plot_zapline_adaptive_summary(obj, channel_names=names, show=False)
        assert isinstance(fig, plt.Figure)
    ...
    def test_no_sfreq_fallback(self):
        ...
        obj = MockAdaptiveZapLine()
        obj.sfreq = None
        fig = plot_zapline_adaptive_summary(obj, sfreq=None, show=False)
        assert isinstance(fig, plt.Figure)
    ...
    def test_save_to_file(self, tmp_path):
        ...
        fig = plot_zapline_adaptive_summary(obj, show=False, fname=str(fpath))
        assert isinstance(fig, plt.Figure)
        assert fpath.exists()
...
class TestPlotZaplineSummary:
    ...
    def test_delegates_to_adaptive(self):
        """Adaptive results should render through the adaptive summary plot."""
        obj = MockAdaptiveZapLine()
        fig = plot_zapline_summary(obj, show=False)
        assert isinstance(fig, plt.Figure)
...
def test_plot_zapline_cleaning_summary(zapline_data, fitted_zapline):
    """Test ZapLine cleaning summary plot."""
    data, sfreq = zapline_data
    cleaned = fitted_zapline.transform(data)

    fig = plot_zapline_cleaning_summary(
        data, cleaned, fitted_zapline, sfreq, show=False
    )
    assert isinstance(fig, plt.Figure)
