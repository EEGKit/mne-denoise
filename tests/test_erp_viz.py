"""Tests for mne_denoise.viz.erp and mne_denoise.viz.erp_io modules.

Covers all 9 public ERP plot functions, the 6 private helpers in erp.py,
the 3 IO functions + ERPGroupData in erp_io.py, and the _finalize_fig
mkdir fix in _theme.py.

Target: >90 % line+branch coverage on erp.py, erp_io.py, and _theme.py.
"""
...
class TestPrivateHelpers:
    """Tests for _pipe_color, _pipe_label, _add_erp_windows, etc."""
    ...
    def test_add_erp_windows_defaults(self):
        _, ax = plt.subplots()
        n_before = len(ax.patches)
        _add_erp_windows(ax)
        # Should have 3 patches (N1, MMN, P300)
        assert len(ax.patches) - n_before == 3

    def test_add_erp_windows_custom(self):
        _, ax = plt.subplots()
        n_before = len(ax.patches)
        _add_erp_windows(ax, {"Custom": (0.1, 0.2)})
    ...
    def test_ch_index_from_epochs(self, epochs_dict):
        idx = _ch_index(epochs_dict["C0"], "Pz")
        assert idx == CH_NAMES.index("Pz")

    def test_ch_index_from_int_string(self):
        idx = _ch_index(np.zeros((5, 10)), "2")
        assert idx == 2

    def test_get_times_ms(self, epochs_dict):
        t = _get_times_ms(epochs_dict["C0"])
        assert t[0] == pytest.approx(-200.0, abs=1.0)

    def test_get_times_ms_raises_on_plain_array(self):
...
class TestPlotGrandAverageERP:
    @pytest.fixture()
    def all_evokeds(self, evokeds_dict):
        """Build {pipe: [Evoked, Evoked, ...]} for 4 fake subjects."""
        out = {}
        for ptag, ev in evokeds_dict.items():
            out[ptag] = [ev, ev, ev, ev]
        return out
